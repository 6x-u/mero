...
--[[
	...
	MERO is a debugging suite designed to help the user debug games and find any potential vulnerabilities.
...
Main = (function()
	...
	Main.Apps = Apps
	Main.MenuApps = {}
	Main.GitRepoName = "6x-u/MERO"
	Main.Developer = "MERO"
	Main.Contact = "TG@QP4RM"
	...
	Main.LoadSettings = function()
		local s,data = pcall(env.readfile or error,"MEROSettings.json")
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="MERO",TextColor3=Color3.new(1,1,1),TextSize=50,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developer: MERO",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
			{12,"UIGradient",{Parent={11},Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1,0),NumberSequenceKeypoint.new(1,1,0),}),}},
			{13,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Version",Parent={2},Position=UDim2.new(1,-110,1,-35),Size=UDim2.new(0,105,0,20),Text="0.1.0",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "X" or "MERO"
...
Main.ScriptDownloader = {}

Main.ScriptDownloader.DownloadAllScripts = function()
	if not env.writefile or not env.makefolder then
		warn("Filesystem not available")
		return
	end
	
	local timestamp = os.date("%Y%m%d_%H%M%S")
	local folderPath = "dex/scripts_" .. timestamp
	
	makefolder("dex")
	makefolder(folderPath)
	
	local scriptCount = 0
	local function saveScript(obj, path)
		if obj:IsA("LocalScript") or obj:IsA("Script") or obj:IsA("ModuleScript") then
			local scriptType = obj.ClassName
			local fileName = path .. "/" .. obj.Name .. "_" .. scriptType .. ".lua"
			
			local success, source = pcall(function()
				return decompile and decompile(obj) or "-- Protected Script"
			end)
		...
		for _, child in pairs(obj:GetChildren()) do
			saveScript(child, path .. "/" .. obj.Name)
		end
	end
	
	for _, service in pairs(game:GetChildren()) do
		saveScript(service, folderPath .. "/" .. service.Name)
	end
...
Export Date: %s
Total Scripts: %d
]], os.date("%Y-%m-%d %H:%M:%S"), scriptCount)
	
	writefile(folderPath .. "/info.txt", infoContent)
	
	print("Scripts saved to: " .. folderPath)
	print("Total scripts exported: " .. scriptCount)
...
Main.AdminManager.GiveAdmin = function()
	local success, result = pcall(function()
		...
		local commands = {
			{name = "fly", desc = "Enable flying"},
			{name = "noclip", desc = "Enable noclip"},
			{name = "god", desc = "God mode"},
			{name = "speed", desc = "Increase speed"},
		}
		
		print("Admin privileges activated for: " .. player.Name)
	...
	if success then
		...
		warn("Failed to give admin privileges")
		return false
	end
end
...
Main.ServerInfo.DisplayInfo = function()
	local info = {
		GameName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name,
		PlaceId = game.PlaceId,
		JobId = game.JobId,
		Players = #game.Players:GetPlayers() .. "/" .. game.Players.MaxPlayers,
		ServerAge = math.floor(workspace.DistributedGameTime / 60) .. " minutes",
		FPS = math.floor(1 / game:GetService("RunService").RenderStepped:Wait()),
		Ping = math.floor(game.Players.LocalPlayer:GetNetworkPing() * 1000) .. "ms",
	}
...
_G.MERO = {
	DownloadScripts = Main.ScriptDownloader.DownloadAllScripts,
	GiveAdmin = Main.AdminManager.GiveAdmin,
	ServerInfo = Main.ServerInfo.DisplayInfo,
...
print("Contact: TG@QP4RM")
print("Use _G.MERO for enhanced features")
